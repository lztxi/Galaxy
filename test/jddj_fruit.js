...
let cookies = [], notify = ''; waterNum = 0, waterTimes = 0, shareCode = '', hzstr = '', msgStr = '';
!(async () => {
    if (cookies.length == 0) {
        if ($.env.isNode) {
    ...
    return new Promise(async resolve => {
        try {
            ...
            if (new Date().getHours() < 8) {
                await $.http.get({ url: 'https://gitee.com/passerby-b/javascript/raw/master/test/sharecode.js' }).then(response => { codestr = response.body; });
                if ($.read('shareCodes')) codestr += $.read('shareCodes');
            }
            try {
                await $.http.get({ url: 'http://51.15.187.136:8080/queryJddjCode', timeout: 20000 }).then(response => { codestr += response.body; });
            } catch (error) { }
...
//逛一逛,带时间逛一逛,定时领水滴,首页访问任务,每日水滴奖励
            ...
            await  $.http.get(option).then(async response => {
                ...
                if (response.body.indexOf('请求成功') > -1) {
                    ...
                    if ($.isNode) {
                        ...
                        let setcookie = JSON.parse(response.headers)['set-cookie'];
                        ckstr = setcookie[0].split(';')[0] + ';' + setcookie[3].split(';')[0] + ';deviceid_pdj_jd=' + deviceid;
                        shareCode = setcookie[3].split(';')[0].split('=')[1];
                    }
                }
